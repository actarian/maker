...
function () {
  ...
  _createClass(App, [{
    ...
    value: function init() {
      ...
      var save = document.querySelector('.btn--export');
      Promise.all([this.getOutputs(), this.getInputs()]).then(function (datas) {
        _this.onSave = _this.onSave.bind(_this);
        save.addEventListener('click', _this.onSave);
      });
    }
  }, {
    key: "getOutputs",
    value: function getOutputs() {
      ...
      return fetch('./api/outputs').then(function (response) {
        return response.json();
      }).then(function (json) {
        _this2.outputs = json.filter(function (x) {
          return x.indexOf('.png') !== -1;
        });
        console.log(_this2.outputs);
      });
    }
  }, {
    key: "getInputs",
    value: function getInputs() {
      ...
      return fetch('./api/inputs').then(function (response) {
        return response.json();
      }).then(function (json) {
        _this3.inputs = json;
      ...
      if (this.items.length) {
        ...
        var icon = fetch("icons/".concat(item.icon)).then(function (response) {
          ...
          _this5.toCanvas(card).then(function (canvas) {
            ...
            _this5.saveToDisk(canvas, item.name).then(function (saved) {
              setTimeout(function () {
                _this5.onNext();
              }, 400);
            });
    ...
    key: "saveToDisk",
    value: function saveToDisk(canvas) {
    ...
    value: function download(canvas) {
      var filename = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'snapshot.png';
      var dataUrl = canvas.toDataURL('image/png', 0.92); // console.log('dataUrl', dataUrl);
